...
{
	/*if (const auto im = imgui::get(); im && im->is_initialized()) {
...
bool get_constant_name(IDirect3DDevice9* dev, UINT start_register)
{
	...
	if (auto hr = dev->GetVertexShader(&used_vs); used_vs)
	{
		...
		{
			const auto constant = constant_table->GetConstant(0, i);
			if (constant)
			{
...
void matrix3x4_transpose_to_4x4(const float3x4* input, D3DXMATRIX* output, const int count)
{
	for (int i = 0; i < count; ++i)
	{
		const float3x4& in = input[i];
...
{
	static uint32_t retn_addr = 0xE694D6;
	__asm
	{
		call	direct3d_create9_stub;
		jmp		retn_addr;
	}
}
